...
build-bin: fmt vet ## Build binary
	@go build -o $(BIN_FILENAME) github.com/appuio/promtool-jsonnet

.PHONY: build-docker
build-docker: build-bin ensure-prometheus ## Build docker image
	$(DOCKER_CMD) build -t $(CONTAINER_IMG) .
	$(DOCKER_CMD) build -t $(CONTAINER_IMG_TEST) -f Dockerfile.test .
